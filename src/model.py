"""Phase 5, 6, 7, 8 & 8.2: Machine Learning Modeling Pipeline for Stay-Tuned.
...
7. Out-of-fold probability generation and decision threshold analysis (Phase 8.2).
8. Final model and threshold selection, full training set refitting, and artifact persistence.

Anti-Leakage Architecture:
--------------------------
- X_test and y_test are NEVER accessed during model selection, cross-validation, tuning,
...
- Out-of-fold (OOF) predictions are generated using StratifiedKFold cross-validation so each
  training instance is predicted by a pipeline trained without that instance.
- The holdout test set (X_test, y_test) remains strictly sealed for Phase 9 final evaluation.
"""
...
# ==============================================================================
# PHASE 8.2: OUT-OF-FOLD THRESHOLD ANALYSIS & FINAL MODEL SELECTION
# ==============================================================================

def generate_oof_predictions(X_train, y_train, tuned_models, cv=None):
    """Generate out-of-fold (OOF) probability predictions on X_train for tuned candidate models.

    Why OOF predictions are used:
    -----------------------------
    Evaluating decision thresholds using in-sample training predictions causes optimistic bias.
    Conversely, evaluating thresholds on X_test leaks test data into the selection process.
    Out-of-fold (OOF) predictions provide an unbiased probability distribution derived strictly
    from X_train: each learner's predicted probability is generated by a pipeline trained on
    the 4 cross-validation folds that did NOT include that learner.

    Why the test set (X_test, y_test) is NOT used:
    ----------------------------------------------
    The holdout test set represents unseen future cohorts. Using X_test to select models or
    calibrate decision thresholds violates statistical independence, creating optimistic bias.
    All model selection, hyperparameter tuning, and threshold calibration must use X_train only.
    ...
    y_train : pandas.Series
        Training target vector.
    tuned_models : dict
        Dictionary containing tuned candidate pipelines ('random_forest' and 'xgboost').
    cv : StratifiedKFold, optional
        Cross-validation generator. Defaults to 5-Fold StratifiedKFold(shuffle=True, random_state=42).

    Returns
    -------
    dict
        Dictionary mapping model names ('random_forest', 'xgboost') to their 1D numpy array
        of out-of-fold predicted probabilities for the positive class (disengaged).
    """
    if cv is None:
    ...
    oof_predictions = {}
    for name in ["random_forest", "xgboost"]:
        pipeline = tuned_models[name]["pipeline"]
        # The pipeline ensures preprocessing is fitted separately inside each CV fold
...
def evaluate_thresholds(y_true, oof_probs, thresholds=None):
    """Evaluate classification performance across multiple probability thresholds on OOF predictions.

    Why threshold 0.5 is NOT automatically assumed:
    -----------------------------------------------
    The conventional 0.5 threshold assumes symmetric misclassification penalties and balanced classes.
    In early learner disengagement prediction:
    1. Severe Class Imbalance: Only ~14.5% of students disengage. At threshold 0.5, models default
       to conservative predictions, missing many at-risk students.
    2. Asymmetric Costs: Missing an at-risk student (False Negative) leads to irreversible course
       withdrawal or failure. Sending a supportive check-in email to an engaged student (False Positive)
       has minimal operational cost.
    Evaluating thresholds enables calibrating sensitivity to the institution's intervention capacity.

    Why recall matters for this early-warning system:
    -------------------------------------------------
    The core value proposition of an early-warning system is prevention. High recall ensures that
    the vast majority of learners heading toward disengagement are caught by Day 14 while there is
    still time to intervene before the Week 3 assessment milestone (TMA 01 due on Day 19).

    Parameters
    ----------
    y_true : pandas.Series or array-like
        Ground truth binary target vector (N=297).
    oof_probs : array-like
        Out-of-fold predicted probabilities for positive class (disengaged).
    thresholds : list of float, optional
        List of decision thresholds to evaluate. Defaults to [0.30, 0.35, 0.40, 0.45, 0.50].

    Returns
    -------
    pandas.DataFrame
        Threshold evaluation summary with columns:
        ['threshold', 'recall', 'precision', 'f1', 'flagged_count', 'flagged_pct']
    """
    ...
    for t in thresholds:
        ...
        flagged_pct = float(flagged / len(y_true_arr) * 100)
...
def select_final_model(
    ...
    max_flagged_pct=30.0,
    min_precision=0.30,
):
    """Programmatically search and select the winning model and operational threshold.

    Selection Criteria & Algorithmic Rule:
    --------------------------------------
    1. Early-Warning Priority:
       Missing an actually disengaged learner (False Negative) is significantly more costly than
       sending an unnecessary supportive check-in (False Positive). Therefore, sensitivity/recall
       is the primary optimization target (target_min_recall >= 70%).
    2. Operational Feasibility (Workload Constraint):
       Advising staff and academic tutors have finite intervention capacity. The model must not flag
       more than max_flagged_pct (default: <= 30.0%) of the cohort at Day 14.
    3. Actionable Precision:
       Interventions should maintain actionable signal quality (min_precision >= 30.0%) to prevent
       advising fatigue and ensure high intervention relevance.
    4. Programmatic Selection:
       The function pools all evaluated threshold configurations from both models, filters those
       meeting the operational constraints, and selects the candidate with the highest recall
       (breaking ties with F1-score, precision, and cross-validation discrimination).

    Parameters
    ----------
    tuned_models : dict
        Dictionary of tuned models returned by tune_candidate_models.
    cv_comparison_df : pandas.DataFrame
        Phase 8 cross-validation comparison results.
    rf_threshold_df : pandas.DataFrame
        Random Forest threshold evaluation table.
    xgb_threshold_df : pandas.DataFrame
        XGBoost threshold evaluation table.
    target_min_recall : float, default=0.70
        Minimum acceptable recall for the early warning system.
    max_flagged_pct : float, default=30.0
        Maximum percentage of students that can be flagged for intervention.
    min_precision : float, default=0.30
        Minimum acceptable precision for flagged alerts.

    Returns
    -------
    dict
        Selection summary containing:
        - 'model_name': str
        - 'pipeline': Pipeline
        - 'threshold': float
        - 'reason': str
        ...
        - 'cv_pr_auc': float
    """
    # 1. Pool all threshold candidates across both candidate models
    rf_candidates = rf_threshold_df.copy()
    rf_candidates["model_name"] = "random_forest"

    xgb_candidates = xgb_threshold_df.copy()
    xgb_candidates["model_name"] = "xgboost"

    all_candidates = pd.concat([rf_candidates, xgb_candidates], ignore_index=True)

    # 2. Filter candidates satisfying operational feasibility constraints
    feasible = all_candidates[
        (all_candidates["flagged_pct"] <= max_flagged_pct)
        & (all_candidates["precision"] >= min_precision)
    ].copy()

    # 3. Apply early-warning selection rule:
    # First priority: candidates meeting target minimum recall
    high_recall_candidates = feasible[feasible["recall"] >= target_min_recall]

    if not high_recall_candidates.empty:
        # Select candidate with highest recall, breaking ties on F1 and precision
        best_candidate = (
            high_recall_candidates
            .sort_values(by=["recall", "f1", "precision"], ascending=[False, False, False])
            .iloc[0]
        )
        selection_path = "met all primary early-warning constraints (recall >= 70%, flagged <= 30%, precision >= 30%)"
    elif not feasible.empty:
        # Fallback: among feasible candidates (flagged <= 30%), select the one maximizing recall
        best_candidate = (
            feasible
            .sort_values(by=["recall", "f1", "precision"], ascending=[False, False, False])
            .iloc[0]
        )
        selection_path = "maximized recall among all feasible configurations (flagged <= 30%, precision >= 30%)"
    else:
        # Emergency fallback: maximize F1 across all candidates
        best_candidate = (
            all_candidates
            .sort_values(by=["f1", "recall"], ascending=[False, False])
            .iloc[0]
        )
        selection_path = "maximized F1 score across all candidate configurations"

    selected_model_name = str(best_candidate["model_name"])
    selected_threshold = float(best_candidate["threshold"])
    selected_pipeline = tuned_models[selected_model_name]["pipeline"]
    selected_params = tuned_models[selected_model_name]["best_params"]
    selected_cv_pr_auc = float(tuned_models[selected_model_name]["best_cv_pr_auc"])

    # Retrieve CV discrimination metrics for the selected model architecture
    cv_row = cv_comparison_df[
        cv_comparison_df["model"] == f"{selected_model_name}_tuned"
    ].iloc[0]

    reason = (
        f"Programmatic search evaluated {len(all_candidates)} candidate threshold configurations across tuned "
        f"Random Forest and XGBoost against operational constraints (target recall >= {target_min_recall*100:.0f}%, "
        f"max flagged cohort <= {max_flagged_pct:.1f}%, min precision >= {min_precision*100:.0f}%). "
        f"Winning configuration: '{selected_model_name}' at threshold {selected_threshold:.2f} ({selection_path}), "
        f"achieving {best_candidate['recall']*100:.2f}% OOF recall, {best_candidate['precision']*100:.2f}% precision, "
        f"and {best_candidate['f1']:.4f} F1, flagging {int(best_candidate['flagged_count'])} students "
        f"({best_candidate['flagged_pct']:.2f}% of cohort). "
        f"In cross-validation, {selected_model_name} demonstrated superior discrimination "
        f"(ROC-AUC: {cv_row['roc_auc_mean']:.4f} +/- {cv_row['roc_auc_std']:.4f}, "
        f"PR-AUC: {cv_row['pr_auc_mean']:.4f} +/- {cv_row['pr_auc_std']:.4f})."
    )

    print("=" * 80)
    print("PHASE 8.2: FINAL MODEL & THRESHOLD SELECTION DECISION")
    print("=" * 80)
    ...
    return {
        ...
        "cv_pr_auc": selected_cv_pr_auc,
    }


def fit_and_save_final_model(selected_model_info, X_train, y_train, output_path=None):
    """Refit the selected model pipeline on all training data and save the complete artifact.

    Parameters
    ----------
    selected_model_info : dict
        Information dictionary returned by select_final_model.
    X_train : pandas.DataFrame
        Full training feature matrix (N=297).
    y_train : pandas.Series
        Full training target vector.
    output_path : Path or str, optional
        Target path for joblib artifact. Defaults to models/best_model.joblib.
    ...
    if output_path is None:
        output_path = PROJECT_ROOT / "models" / "best_model.joblib"
    else:
        output_path = Path(output_path)

    output_path.parent.mkdir(parents=True, exist_ok=True)

    # Refit the chosen pipeline on the entire training set (X_train, y_train)
    # Note: X_test and y_test are NEVER touched or passed here.
    pipeline = clone(selected_model_info["pipeline"])
    pipeline.fit(X_train, y_train)

    artifact = {
        "pipeline": pipeline,
        "threshold": float(selected_model_info["threshold"]),
        "model_name": selected_model_info["model_name"],
        "feature_names": list(X_train.columns),
        "best_params": selected_model_info.get("best_params"),
        "cv_pr_auc": selected_model_info.get("cv_pr_auc"),
        "training_samples": len(X_train),
        "training_positive_cases": int((y_train == 1).sum()),
    }

    joblib.dump(artifact, output_path)
    print(f"Final model artifact saved successfully to: {output_path}")
    return output_path


def print_phase8_2_validation_report(
    X_train,
    y_train,
    selected_model_info,
    saved_path,
    ...
    """Print the official Phase 8.2 validation report confirming anti-leakage and artifact integrity."""
    print("=" * 80)
    print("PHASE 8.2: FINAL MODEL SELECTION & THRESHOLD ANALYSIS VALIDATION REPORT")
    print("=" * 80)
    print(f"1. Training Set Size (X_train):            {len(X_train)} students")
    print(f"2. Training Target Distribution (y_train):   Class 0: {(y_train == 0).sum()} ({(y_train == 0).mean()*100:.2f}%), Class 1: {(y_train == 1).sum()} ({(y_train == 1).mean()*100:.2f}%)")
    print("3. Holdout Test Set Accessed:              False (X_test and y_test strictly untouched)")
    print(f"4. Selected Model Architecture:            {selected_model_info['model_name']}")
    print(f"5. Selected Operational Threshold:         {selected_model_info['threshold']:.2f}")
    print(f"6. Saved Model Artifact Path:              {saved_path}")
    print("-" * 80)

    print("Out-of-Fold Threshold Analysis - Random Forest:")
    rf_formatted = rf_threshold_df.copy()
    rf_formatted["recall"] = rf_formatted["recall"].apply(lambda v: f"{v*100:5.2f}%")
    rf_formatted["precision"] = rf_formatted["precision"].apply(lambda v: f"{v*100:5.2f}%")
    rf_formatted["f1"] = rf_formatted["f1"].apply(lambda v: f"{v:.4f}")
    rf_formatted["flagged_pct"] = rf_formatted["flagged_pct"].apply(lambda v: f"{v:5.2f}%")
    print(rf_formatted.to_string(index=False))

    print("\nOut-of-Fold Threshold Analysis - XGBoost:")
    xgb_formatted = xgb_threshold_df.copy()
    xgb_formatted["recall"] = xgb_formatted["recall"].apply(lambda v: f"{v*100:5.2f}%")
    xgb_formatted["precision"] = xgb_formatted["precision"].apply(lambda v: f"{v*100:5.2f}%")
    xgb_formatted["f1"] = xgb_formatted["f1"].apply(lambda v: f"{v:.4f}")
    xgb_formatted["flagged_pct"] = xgb_formatted["flagged_pct"].apply(lambda v: f"{v:5.2f}%")
    print(xgb_formatted.to_string(index=False))
    print("-" * 80)

    # Assertions
    assert saved_path.exists(), f"Artifact {saved_path} does not exist!"
    loaded_artifact = joblib.load(saved_path)
    assert "pipeline" in loaded_artifact, "Saved artifact missing 'pipeline'"
    assert "threshold" in loaded_artifact, "Saved artifact missing 'threshold'"
    assert loaded_artifact["threshold"] == selected_model_info["threshold"], "Threshold mismatch"
    assert hasattr(loaded_artifact["pipeline"].named_steps["classifier"], "classes_"), "Pipeline not fitted"

    print("ALL PHASE 8.2 CHECKS PASSED: Model and threshold selected based on early-warning objectives.")
    print("Holdout test set remains completely sealed for Phase 9 final evaluation.")
    print("============================================================\n")
...
if __name__ == "__main__":
    ...
    # Ensure fresh execution: remove any existing best_model.joblib before Phase 8
    premature_model_path = PROJECT_ROOT / "models" / "best_model.joblib"
    if premature_model_path.exists():
        premature_model_path.unlink()
    ...
    # 16. Run Phase 8.2: Generate Out-of-Fold (OOF) Probability Predictions
    ...
    # 17. Evaluate Decision Thresholds across OOF Predictions
    rf_threshold_df = evaluate_thresholds(
        y_true=y_train,
        oof_probs=oof_predictions["random_forest"],
    )
    xgb_threshold_df = evaluate_thresholds(
        y_true=y_train,
        oof_probs=oof_predictions["xgboost"],
    )

    # 18. Select Final Model and Operational Decision Threshold
    selected_model_info = select_final_model(
        tuned_models=tuned_models,
        cv_comparison_df=tuned_comparison_df,
        rf_threshold_df=rf_threshold_df,
        xgb_threshold_df=xgb_threshold_df,
    )

    # 19. Refit Selected Model on Complete X_train and Save Artifact
    saved_artifact_path = fit_and_save_final_model(
        selected_model_info=selected_model_info,
        X_train=X_train,
        y_train=y_train,
    )

    # 20. Print official Phase 8.2 validation report
    print_phase8_2_validation_report(
        X_train=X_train,
        y_train=y_train,
        selected_model_info=selected_model_info,
        saved_path=saved_artifact_path,
        rf_threshold_df=rf_threshold_df,
        xgb_threshold_df=xgb_threshold_df,
    )
